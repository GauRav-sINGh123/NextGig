  ...
  const handleUploadResume = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/auth/upload_resume`,
        formData,
        {
    ...
    >
      ...
      ${
        isLoading
          ...
          : "cursor-pointer bg-blue-500/10 hover:bg-blue-500/20 text-blue-400"
      }`}
              >
                {user?.profile?.resume ? "Edit" : "Upload"}
            ...
            {user?.profile?.resume ? (
              <a
                ...
                className="block bg-white/5 rounded-xl p-4 hover:bg-white/10 transition"
              >
                <div className="flex items-center">
                  <FileText className="w-8 h-8 text-blue-500 mr-3" />
                  <div>
                    <p className="font-medium">{user.profile.resumeName}</p>
                    <p className="text-xs text-white/40">Click to view</p>
                  </div>
                </div>
              </a>
            ) : (
